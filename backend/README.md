# MuchToDo API (StartTech Backend)

A robust RESTful API for the StartTech / MuchToDo ToDo application built with Go (Golang). This service backs the React frontend and is deployed behind an AWS Application Load Balancer (ALB) and CloudFront.

The API provides user authentication, JWT-based session management, CRUD operations for ToDo items, and an optional Redis caching layer. It uses a clean, layered architecture and includes unit and integration tests plus Swagger documentation.
...
### 3. Start Local Dependencies (MongoDB, Redis, API)
...
* `ENABLE_CACHE`, `REDIS_ADDR` – Redis caching.
* `JWT_SECRET_KEY`, `JWT_EXPIRATION_HOURS` – authentication.
* `ALLOWED_ORIGINS` – allowed CORS origins (includes the deployed frontend CloudFront URL).
* `COOKIE_DOMAINS`, `SECURE_COOKIE` – cookie behaviour for HTTPS.

In production, the API is typically:

* Deployed on EC2 (or containers) behind an **Application Load Balancer (ALB)**.
* Exposed to the public Internet via a **CloudFront distribution** configured with:
  * Origin: ALB DNS name.
  * Viewer protocol policy: redirect HTTP to HTTPS.
  * Allowed methods: `GET, HEAD, OPTIONS, PUT, POST, PATCH, DELETE`.

The frontend is configured with `VITE_API_BASE_URL` pointing at the backend CloudFront URL so that browser requests are fully HTTPS and respect CORS.